...
export const pageDataArchitectureProcessorOperations: PageDataArchitectureProcessorOperations =
  {
    addProcessorOperation(state, action) {
      const endpoint = state.endpoints.find(
        (e) => e.id === action.input.endpointId,
      );
      if (!endpoint) {
        throw new EndpointNotFoundError(
          `Endpoint with ID ${action.input.endpointId} not found`,
        );
      }
    ...
    updateProcessorOperation(state, action) {
      ...
      const processor = endpoint.processors.find(
        (p) => p.id === action.input.id,
      );
      if (!processor) {
        throw new ProcessorNotFoundError(
          `Processor with ID ${action.input.id} not found`,
        );
      }
      ...
      if (
        ...
        processor.description = action.input.description;
    ...
    deleteProcessorOperation(state, action) {
      ...
      const index = endpoint.processors.findIndex(
        (p) => p.id === action.input.id,
      );
      if (index === -1) {
        throw new ProcessorNotFoundError(
    ...
    addColumnOperation(state, action) {
      ...
      const processor = endpoint.processors.find(
        (p) => p.id === action.input.processorId,
      );
      if (!processor) {
        throw new ProcessorNotFoundError(
          `Processor with ID ${action.input.processorId} not found`,
        );
      }
    ...
    updateColumnOperation(state, action) {
      ...
      if (!column) {
        throw new ColumnNotFoundError(
          `Column with ID ${action.input.id} not found`,
        );
      }
      if (action.input.name) column.name = action.input.name;
      if (action.input.dataType) column.dataType = action.input.dataType;
      if (
        action.input.isPrimaryKey !== undefined &&
        action.input.isPrimaryKey !== null
      )
        column.isPrimaryKey = action.input.isPrimaryKey;
      if (
        action.input.isNullable !== undefined &&
        action.input.isNullable !== null
      )
        column.isNullable = action.input.isNullable;
      if (
        action.input.defaultValue !== undefined &&
        action.input.defaultValue !== null
      )
        column.defaultValue = action.input.defaultValue;
      if (
        action.input.references !== undefined &&
        action.input.references !== null
      )
        column.references = action.input.references;
    },
    deleteColumnOperation(state, action) {
      ...
      const index = processor.columns.findIndex(
