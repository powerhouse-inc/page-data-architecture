...
export const pageDataArchitectureSubgraphOperations: PageDataArchitectureSubgraphOperations =
  {
    ...
    updateSubgraphOperation(state, action) {
      ...
      if (!subgraph) {
        throw new SubgraphNotFoundError(
          `Subgraph with ID ${action.input.id} not found`,
        );
      }
      ...
      if (
        ...
        subgraph.description = action.input.description;
      if (action.input.graphqlSchema)
        subgraph.graphqlSchema = action.input.graphqlSchema;
      if (action.input.graphqlQuery)
        subgraph.graphqlQuery = action.input.graphqlQuery;
    },
    deleteSubgraphOperation(state, action) {
      ...
      const index = endpoint.subgraphs.findIndex(
    ...
    linkSubgraphToProcessorOperation(state, action) {
      ...
      const subgraph = endpoint.subgraphs.find(
        (s) => s.id === action.input.subgraphId,
      );
      if (!subgraph) {
        throw new SubgraphNotFoundError(
          `Subgraph with ID ${action.input.subgraphId} not found`,
        );
      }
    ...
    unlinkSubgraphFromProcessorOperation(state, action) {
      ...
      const processorIndex = subgraph.processorIds.indexOf(
        action.input.processorId,
      );
      if (processorIndex !== -1) {
